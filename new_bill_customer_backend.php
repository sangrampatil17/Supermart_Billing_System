...
if(isset($_POST['delete_qty'])){
    ...
    if(isset($_POST['generate_bill'])){
        ...
        $invoice_no++;
        $query="SELECT * FROM customer_bill_details WHERE basket_no='$basket_no' AND bill_no IS NULL AND bill_date IS NULL AND basket_no='0' ORDER BY id DESC";
        $result=$con->query($query);
        ...
        if($result->num_rows>0){
            while($row=$result->fetch_assoc()){
                ...
                $con->query("UPDATE customer_bill_details SET bill_no='$invoice_no',bill_date='$billdate' WHERE basket_no='$basket_no' AND bill_no IS NULL AND bill_date IS NULL AND basket_no='0'");
